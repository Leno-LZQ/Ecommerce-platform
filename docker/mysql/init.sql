...
CREATE TABLE `permission` (
    ...
    `name`        VARCHAR(100) NOT NULL COMMENT '权限标识（如 product:create）',
    `description` VARCHAR(200) DEFAULT NULL COMMENT '权限描述',
    `create_time` DATETIME     NOT NULL COMMENT '创建时间',
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_name` (`name`) COMMENT '权限标识唯一'
...
CREATE TABLE `user` (
    ...
    `phone`       VARCHAR(20)  DEFAULT NULL COMMENT '手机号（AES-256 加密存储）',
...
-- ============================================================
-- 域 2（续）：商品表（FK → category, merchant）
-- ============================================================
...
CREATE TABLE `product` (
    ...
    `merchant_id`    BIGINT        NOT NULL COMMENT '所属商家ID',
    `price`          DECIMAL(10,2) DEFAULT 0.00 COMMENT '默认价格（最低SKU价格）',
    `stock`          INT           DEFAULT 0 COMMENT '总库存（缓存字段）',
...
-- ============================================================
-- 域 2（续）：商品评价表（FK → product, sku, user, orders）
-- 延迟创建：被引用的 order_no 先存在于 orders 表
-- ============================================================
